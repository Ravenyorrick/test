...
- `NativeAudioCapture` using CPAL for native microphone enumeration, selected-device opening, stream start/stop, mono frame conversion, and RMS/peak/drop metrics.
- Native CLI bridge:
  - `voxshift-audio-engine enumerate-devices`
  - `voxshift-audio-engine capture-test --device <id> --duration-ms <ms>`
