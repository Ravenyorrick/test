...
ipcMain.handle("extract:start", async (_event, url: string, apiKey?: string, options?: { perPage?: number; autoEnrich?: boolean; revealPersonalEmails?: boolean; enrichmentConcurrency?: number; maxEmailRevealsPerRun?: number }) => {
  ...
  if (resolvedApiKey) {
    return apiRunner.run(url, resolvedApiKey, emit, options?.perPage, options?.autoEnrich, options?.revealPersonalEmails, options?.enrichmentConcurrency, options?.maxEmailRevealsPerRun);
  }
